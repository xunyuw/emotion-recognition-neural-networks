...
X = np.load("data_set_extended.npy")
X = np.reshape(X, [-1, 50, 50, 1])
X_test = np.load("test_set_extended.npy")
X_test = np.reshape(X_test, [-1, 50, 50, 1])
Y = np.load("data_labels_extended.npy")
Y_test = np.load("test_labels_extended.npy")
...
network = conv_2d(network, 80, 3, activation='relu')
network = conv_2d(network, 80, 3, activation='relu')
network = max_pool_2d(network, 2)
network = fully_connected(network, 320, activation='relu')
